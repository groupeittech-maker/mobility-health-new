...
function renderStayList() {
    ...
    const totalPages = Math.max(1, Math.ceil(state.stays.length / ROWS_PER_PAGE));
    if (currentPageStays > totalPages - 1) {
        currentPageStays = Math.max(0, totalPages - 1);
    }
    const start = currentPageStays * ROWS_PER_PAGE;
    const pageStays = state.stays.slice(start, start + ROWS_PER_PAGE);

    list.innerHTML = pageStays.map((stay) => {
    ...
    if (pagEl) {
        if (state.stays.length <= ROWS_PER_PAGE) {
            ...
            `;
            document.getElementById('staysPrev')?.addEventListener('click', () => {
                currentPageStays = Math.max(0, currentPageStays - 1);
                renderStayList();
            });
            document.getElementById('staysNext')?.addEventListener('click', () => {
                currentPageStays = Math.min(totalPages - 1, currentPageStays + 1);
                renderStayList();
            });
        }
    }
...
function renderInvoicePreview(stay) {
    if (!stay) {
        ...
        if (empty) empty.textContent = 'Aucun séjour sélectionné.';
        if (generateBtn) generateBtn.disabled = true;
        if (infoEl) infoEl.hidden = true;
        updateValidationBadges(null);
        return;
    }
    const overrideLines =
        state.invoiceDraft?.stayId === stay.id ? state.invoiceDraft.lines : null;

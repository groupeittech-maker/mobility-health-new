...
function renderSubscriptionDetails(subscription, container) {
    ...
    try {
        ...
        const quoteCur = product && product.currency;
        ...
        container.innerHTML = `
            <div class="details-header">
                ...
                <div class="details-meta">
                    ...
                    <div class="price-chip">${formatMontantDevis(subscription.prix_applique || 0, quoteCur)}</div>
                </div>
            </div>
            ...
            <section class="detail-section">
                ...
                <div class="info-grid">
                    ...
                    ${renderInfoCard('Prix appliqué', formatMontantDevis(subscription.prix_applique || 0, quoteCur))}
    ...
    return `
        <div class="detail-section">
            ...
            <div class="info-grid">
                ...
                ${renderInfoCard('Prix catalogue', formatMontantDevis(product.cout, product.currency))}
    ...
    return `
        <div class="detail-section">
            ...
            <div class="info-grid">
                ...
                ${renderInfoCard('Destination', project.destination_display || project.destination || '—')}
                ...
                ${renderInfoCard('Budget estimé', project.budget_estime ? formatMontantDevis(project.budget_estime, quoteCurrency) : 'Non défini')}
            </div>
...
async function maybeAttachEcardSection(subscription, container) {
    ...
    try {
        ...
        section.innerHTML = renderEcardSection(ecard, subscription.id);
        setTimeout(function() {
            loadSubscriptionEcardViaApi(subscription.id);
            bindSubscriptionEcardDownload(subscription.id, ecard.numero_souscription || 'mobility');
        }, 0);
    } catch (error) {
        if (error?.status === 404) {
...
function renderEcardSection(ecard, subscriptionId) {
    ...
    const sid = subscriptionId != null ? subscriptionId : '';
    ...
    return `
        <div class="ecard-section">
            ...
            <img data-subscription-id="${escapeHtml(String(sid))}" data-ecard-load="1" src="${placeholderSvg}" alt="Carte numérique Mobility Health" class="ecard-image subscription-ecard-img" loading="lazy">
            <div class="info-grid">
                ${metaCards}
            </div>
            <div class="ecard-actions" style="gap: 0.75rem; margin-top: 1rem;">
                <a href="#" class="btn btn-secondary btn-sm subscription-ecard-download" data-subscription-id="${escapeHtml(String(sid))}" data-numero="${escapeHtml(ecard.numero_souscription || 'mobility')}">Télécharger la carte</a>
            </div>
        </div>
    `;
}
...
function loadSubscriptionEcardViaApi(subscriptionId) {
    var img = document.querySelector('.subscription-ecard-img[data-subscription-id="' + subscriptionId + '"]');
    if (!img) return;
    ...
    var url = apiBase + '/subscriptions/' + subscriptionId + '/ecard/download';
...
function bindSubscriptionEcardDownload(subscriptionId, numero) {
    var link = document.querySelector('.subscription-ecard-download[data-subscription-id="' + subscriptionId + '"]');
    if (!link) return;

...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const residence_country_name =
        (user && user.pays_residence && String(user.pays_residence).trim()) || null;
    ...
    payBtn.addEventListener('click', async () => {
        ...
        try {
            const med = forms.medical || {};
            const pm = med.photoMedicale;
            if (!pm || typeof pm !== 'string' || pm.length < 50) {
                showMessage(
                    'La photo pour la e-carte est manquante. Retournez au questionnaire médical ou à la sélection du produit pour ajouter une image.',
                    'error',
                );

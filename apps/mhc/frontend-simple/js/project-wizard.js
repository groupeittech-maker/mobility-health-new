document.addEventListener('DOMContentLoaded', async () => {
    ...
    const residenceCountry = (localStorage.getItem('user_pays_residence') || '').trim();
    if (userName) {
    ...
    function isResidenceDestinationConflict(country) {
        ...
        const residenceCode = normalizeCountryCode(residenceCountry);
        const countryCode = normalizeCountryCode(country.code);
        if (residenceCode && countryCode && residenceCode === countryCode) {
            return true;
        }
    ...
    destinationSelect?.addEventListener('change', function() {
        const selectedCountry = destinationCountriesData.find((country) => country.nom === this.value);
        if (isResidenceDestinationConflict(selectedCountry)) {
            this.value = '';
            if (destinationCitySelect) {
                destinationCitySelect.innerHTML = '<option value="">Choisir un pays d\'abord...</option>';
                destinationCitySelect.disabled = true;
            }
    ...
    form.addEventListener('submit', async (event) => {
        ...
        const destinationCity = (formData.get('destination_city') || '').trim();
        const destinationCountry = (formData.get('destination_country') || '').trim();
        const destination = destinationCity || destinationCountry || 'Destination à préciser';
        const destinationDisplay = buildDestinationDisplay(destinationCity, destinationCountry);
        const selectedCountryOption = destinationSelect?.options[destinationSelect?.selectedIndex];
        const destination_country_id = selectedCountryOption?.dataset?.countryId
            ? parseInt(selectedCountryOption.dataset.countryId, 10)
            : null;
        const destination_country_code = selectedCountryOption?.dataset?.countryCode || '';
        const selectedCountry = destinationCountriesData.find((country) =>
    ...
    function populateCountries(select, countriesData) {
        ...
        countriesData.forEach((country) => {
            ...
            if (isResidenceDestinationConflict(country)) {
                option.disabled = true;
                option.text = `${country.nom} (pays de résidence)`;
            }
    ...
    function buildDestinationDisplay(city, country) {

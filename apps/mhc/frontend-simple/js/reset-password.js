...
function getResetApiBaseUrl() {
    ...
    return 'https://srv1324425.hstgr.cloud/api/v1';
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    try {
        ...
        const usernameOrEmail = params.get('username');
        if (emailInput && usernameOrEmail) {
            emailInput.value = usernameOrEmail;
        }
    } catch (error) {
        console.warn('Impossible de préremplir le champ email:', error);
    }
    ...
    if (verifyCodeForm) {
        verifyCodeForm.addEventListener('submit', async function(e) {
            ...
            try {
                const apiUrl = getResetApiBaseUrl();

...
async function handleSubmit(event) {
    ...
    if (payload.latitude < -90 || payload.latitude > 90) {
        showAlert('La latitude doit être entre -90 et 90 (ex. -4.27 pour Brazzaville).', 'error');
        return;
    }
    if (payload.longitude < -180 || payload.longitude > 180) {
        showAlert('La longitude doit être entre -180 et 180 (ex. 15.27 pour Brazzaville). Vérifiez le format (virgule ou point décimal).', 'error');
        return;
    }

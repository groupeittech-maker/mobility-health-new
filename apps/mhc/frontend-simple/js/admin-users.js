...
const ROLE_OPTIONS = [
    ...
    { value: 'agent_comptable_courtier', label: 'Agent comptable courtier' },
...
async function confirmDeleteUser(userId, label = '') {
    ...
    const confirmed = window.confirm(
        `Supprimer définitivement ${displayName} ?\n\n` +
            'Toutes les données liées à ce compte seront supprimées ou détachées (souscriptions, paiements, projets, alertes SOS, etc.). Cette action est irréversible.'
    );
    if (!confirmed) {
        return;
    }
...
function setupResetPasswordForm() {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        if (!/[a-zA-Z]/.test(newPassword)) {
            showAlert('Le mot de passe doit contenir au moins une lettre (a-z).', 'error');
            return;
        }

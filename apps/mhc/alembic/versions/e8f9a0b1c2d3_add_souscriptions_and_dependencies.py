"""Add souscriptions and dependency tables (assureurs, produits_assurance, projets_voyage, destinations)

This migration creates the tables required before questionnaires/notifications (ad587bb061e5).
Run only on PostgreSQL when souscriptions does not exist (e.g. fresh DB with linear chain).
...
def _create_pg_enums(conn):
    """Create PostgreSQL ENUM types if they do not exist."""
    enums = [
...
def upgrade() -> None:
    ...
    if "souscriptions" in existing:
        return  # Already applied (e.g. from another branch)

    _create_pg_enums(conn)

    statutsouscription = postgresql.ENUM(
        "en_attente", "pending", "active", "suspendue", "resiliee", "expiree",
        name="statutsouscription", create_type=False
    )
    statutprojetvoyage = postgresql.ENUM(
        "en_planification", "confirme", "en_cours", "termine", "annule",
        name="statutprojetvoyage", create_type=False
    )
    questionnairetype = postgresql.ENUM("short", "long", name="questionnairetype", create_type=False)
    clerepartition = postgresql.ENUM(
        "par_personne", "par_groupe", "par_duree", "par_destination", "fixe",
        name="clerepartition", create_type=False
    )
    ...
    if "souscriptions" not in existing:
        op.create_table(
            ...
            sa.PrimaryKeyConstraint("id"),
            sa.UniqueConstraint("numero_souscription"),
        )
...
def downgrade() -> None:
    ...
    if "souscriptions" in existing:
        op.drop_index("ix_souscriptions_demande_resiliation", table_name="souscriptions")
        op.drop_index("ix_souscriptions_validation_finale", table_name="souscriptions")
        op.drop_index("ix_souscriptions_validation_technique", table_name="souscriptions")
        op.drop_index("ix_souscriptions_validation_medicale", table_name="souscriptions")
        op.drop_index("ix_souscriptions_numero_souscription", table_name="souscriptions")
        op.drop_index("ix_souscriptions_projet_voyage_id", table_name="souscriptions")
        op.drop_index("ix_souscriptions_produit_assurance_id", table_name="souscriptions")
        op.drop_index("ix_souscriptions_user_id", table_name="souscriptions")
        op.drop_index("ix_souscriptions_id", table_name="souscriptions")
        op.drop_table("souscriptions")

    if "projets_voyage" in existing:
        op.drop_index("ix_projets_voyage_destination_country_id", table_name="projets_voyage")
        op.drop_index("ix_projets_voyage_user_id", table_name="projets_voyage")
        op.drop_index("ix_projets_voyage_id", table_name="projets_voyage")
        op.drop_table("projets_voyage")

    if "produits_assurance" in existing:
        op.drop_index("ix_produits_assurance_assureur_id", table_name="produits_assurance")
        op.drop_index("ix_produits_assurance_code", table_name="produits_assurance")
        op.drop_index("ix_produits_assurance_id", table_name="produits_assurance")
        op.drop_table("produits_assurance")

    if "destination_cities" in existing:
        op.drop_index("ix_destination_cities_pays_id", table_name="destination_cities")
        op.drop_index("ix_destination_cities_nom", table_name="destination_cities")
        op.drop_index("ix_destination_cities_id", table_name="destination_cities")
        op.drop_table("destination_cities")

    if "destination_countries" in existing:
        op.drop_index("ix_destination_countries_nom", table_name="destination_countries")
        op.drop_index("ix_destination_countries_code", table_name="destination_countries")
        op.drop_index("ix_destination_countries_id", table_name="destination_countries")
        op.drop_table("destination_countries")

    if "assureurs" in existing:
        op.drop_index("ix_assureurs_agent_comptable_id", table_name="assureurs")
        op.drop_index("ix_assureurs_nom", table_name="assureurs")
        op.drop_index("ix_assureurs_id", table_name="assureurs")
        op.drop_table("assureurs")

    for enum_name in ["statutsouscription", "statutprojetvoyage", "questionnairetype", "clerepartition"]:
        try:
            op.execute(sa.text("DROP TYPE IF EXISTS {} CASCADE".format(enum_name)))
        except Exception:
            pass

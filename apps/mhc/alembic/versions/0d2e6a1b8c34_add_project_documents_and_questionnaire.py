...
Idempotent: skip adding questionnaire_type / creating table if already present (e.g. from ad587bb061e5).
"""
...
def upgrade() -> None:
    ...
    if "postgresql" in dialect_name:
        ...
        # Idempotent: add column only if missing (e.g. already created by ad587bb061e5)
        r = bind.execute(sa.text(
            "SELECT 1 FROM information_schema.columns WHERE table_name = 'projets_voyage' AND column_name = 'questionnaire_type'"
        )).fetchone()
        if not r:
            bind.execute(sa.text(
                "ALTER TABLE projets_voyage ADD COLUMN questionnaire_type questionnairetype NOT NULL DEFAULT 'long'"
            ))
            bind.execute(sa.text("ALTER TABLE projets_voyage ALTER COLUMN questionnaire_type DROP DEFAULT"))
        # Skip op.add_column path below for postgres
    else:
        cols = [c["name"] for c in inspector.get_columns("projets_voyage")]
        if "questionnaire_type" not in cols:
            questionnaire_enum = sa.Enum("short", "long", name=QUESTIONNAIRE_ENUM_NAME)
            op.add_column(
                "projets_voyage",
                sa.Column(
    ...
    if "projet_voyage_documents" in existing_tables:
        return
    op.create_table(

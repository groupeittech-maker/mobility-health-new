...
class MinioService:
    ...
    @staticmethod
    def upload_courtier_logo(courtier_id: int, file_bytes: bytes, content_type: str, extension: str = "png") -> str:
        """
        Upload le logo d'un courtier dans MinIO et retourne la clé de l'objet (à stocker en logo_url).
        La clé est de la forme: courtiers/{id}/logo.{ext}
        """
        MinioService.ensure_logos_bucket()
        object_name = f"courtiers/{courtier_id}/logo.{extension.lstrip('.')}"
        try:
            from io import BytesIO
            file_io = BytesIO(file_bytes)
            minio_client.put_object(
            ...
            raise Exception(f"Erreur lors de l'upload du logo courtier sur MinIO: {str(e)}")

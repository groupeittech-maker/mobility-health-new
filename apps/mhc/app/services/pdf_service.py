...
def _mobility_logo_png_to_bytesio(path: str) -> Optional[BytesIO]:
    """PNG → BytesIO ; pour le logo officiel sur fond noir, transparence pour PDF blanc."""
    try:
...
def _garanties_from_product(produit) -> Dict[str, Dict[str, str]]:
    ...
    result = {
        ...
        "rapatriement": {"montant": "N/A", "franchise": "0 FCFA"},
        "assistance_deces": {"montant": "N/A", "franchise": "0 FCFA"},
        "responsabilité_civile": {"montant": "N/A", "franchise": "N/A"},
        "accident_corporel": {"montant": "N/A", "franchise": "0 FCFA"},
    ...
    for g in garanties:
        ...
        franchise = g.get("franchise") or "0 FCFA"
...
def _build_conditions_context(
    ...
    return {
        ...
        "prime_nette": f"{prime_nette:,.2f} FCFA".replace(",", " "),
        "taxes_frais": f"{taxes_frais:,.2f} FCFA".replace(",", " "),
        "total_payer": f"{total_payer:,.2f} FCFA".replace(",", " "),
...
class PDFService:
    ...
    def generate_attestation_definitive(
        ...
        data_paiement = [
            ["Montant payé:", f"{float(paiement.montant):.2f} FCFA"],
        ...
        data_couverture = [
            ...
            ["Prix appliqué:", f"{float(souscription.prix_applique):.2f} FCFA"],
        ]

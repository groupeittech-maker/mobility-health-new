...
def _db_url() -> str:
    return getattr(settings, "DATABASE_URL", None) or os.environ.get("DATABASE_URL", "sqlite:///./mobility_health.db")
...
_cors_raw = getattr(settings, "CORS_ORIGINS", None)
if _cors_raw is None:
    _env_cors = (os.environ.get("CORS_ORIGINS") or "").strip()
    _cors_raw = [o.strip() for o in _env_cors.split(",") if o.strip()] if _env_cors else _default_cors
if isinstance(_cors_raw, str):
    _cors_raw = [_cors_raw] if _cors_raw.strip() else _default_cors
cors_origins = [origin for origin in (_cors_raw if isinstance(_cors_raw, list) else _default_cors) if origin and str(origin).strip() != "*"]

# En développement, autoriser toutes les origines localhost (pour Flutter Web)
_debug = getattr(settings, "DEBUG", False)
_env = (getattr(settings, "ENVIRONMENT", None) or os.environ.get("ENVIRONMENT", "production") or "").lower()
if _debug or _env in {"development", "local", "test"}:
...
logger.info(f"CORS configured for {len(cors_origins)} origins (DEBUG={_debug}, ENV={_env})")
if _debug or _env in {"development", "local", "test"}:
...
is_dev = _debug or _env in {"development", "local", "test"}
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    # Message lisible pour le frontend (évite [object Object])
    parts = []
    for e in errors:
        loc = e.get("loc", ())
        field = ".".join(str(x) for x in loc if x != "body") or "champ"
        msg = e.get("msg", "invalide")
        parts.append(f"{field}: {msg}")
    message = " ; ".join(parts) if parts else "Erreur de validation"
    return JSONResponse(
...
async def _health_response():
    """Réponse JSON du health check (partagée par /health et /api/v1/health)."""

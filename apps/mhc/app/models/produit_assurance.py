...
class ProduitAssurance(Base, TimestampMixin):
    ...
    cle_repartition = Column(
        SQLEnum(CleRepartition, values_callable=lambda x: [e.value for e in x]),
        nullable=False,
        default=CleRepartition.FIXE,
    )
    ...
    # Surprimes âge (%) sur le tarif ligne (produit_prime_tarif). Réf. 18–69 ans : 0 %.
    # prime = tarif_ligne × (1 + pct/100) si la ligne n’a pas de tranche d’âge explicite ; sinon tarif_ligne seul.
    surprime_moins_18_pct = Column(Numeric(6, 2), nullable=True, default=0)
    surprime_70_75_pct = Column(Numeric(6, 2), nullable=True, default=0)
    surprime_76_80_pct = Column(Numeric(6, 2), nullable=True, default=0)
    surprime_81_89_pct = Column(Numeric(6, 2), nullable=True, default=0)
    ...
    grille_finale_lignes = relationship(

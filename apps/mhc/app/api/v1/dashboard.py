...
async def get_dashboard_stats(
    ...
    # Souscriptions en attente de validation (valeurs en minuscules pour l'enum PostgreSQL)
    _pending_statuts = ("en_attente", "pending")
    subscriptions_pending = db.query(Souscription).filter(
        Souscription.statut.in_(_pending_statuts)
    ).count()

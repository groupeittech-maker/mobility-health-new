...
def _enqueue_or_send_push_now(
    *,
    ...
    """
    Tente l'envoi asynchrone via Celery.
    En fallback (broker indisponible), exécute immédiatement la tâche en local.
    """
    from app.workers.tasks import send_notification_multi_channel

    payload_channels = channels or ["push"]
    try:
        ...
        logger.warning(
            "Celery indisponible pour notif=%s user=%s, fallback sync: %s",
            notification_id,
            user_id,
            e,
        )
        try:
            send_notification_multi_channel.apply(
                kwargs={
                    "user_id": user_id,
                    "notification_id": notification_id,
                    "channels": payload_channels,
            ...
            logger.exception(
                "Echec fallback push sync notif=%s user=%s",
                notification_id,
                user_id,
            )


def _normalize_photo_payload_to_url(photo_payload) -> Optional[str]:
    ...
    if isinstance(photo_payload, dict):
        url = (
            photo_payload.get("dataUrl")
            or photo_payload.get("url")
            or photo_payload.get("photo_url")
        )
        if url:
            return url
        b64 = photo_payload.get("base64")
        if b64:
            return f"data:image/jpeg;base64,{b64}"
    return None


def _photo_url_from_administratif_reponses(reponses: dict) -> Optional[str]:
    if not reponses or not isinstance(reponses, dict):
        return None
    personal = reponses.get("personal") or {}
    technical = reponses.get("technical") or {}
    photo_payload = (
        personal.get("photoIdentity")
        or personal.get("photo_identity")
        or technical.get("photoIdentity")
        or technical.get("photo_identity")
        or reponses.get("photoIdentity")
        or reponses.get("photo_identity")
        or reponses.get("identityPhoto")
        or reponses.get("identity_photo")
    )
    return _normalize_photo_payload_to_url(photo_payload)


def _subscription_identity_photo_url(db: Session, subscription_id: Optional[int]) -> Optional[str]:
    """Photo d'identité assuré (e-carte) depuis questionnaires admin / médical."""
    ...
    if q_admin and q_admin.reponses:
        url = _photo_url_from_administratif_reponses(q_admin.reponses)
        if url:
            return url
    q_med = get_latest_questionnaire(db, subscription_id, "medical")
    if q_med and q_med.reponses:
        from app.services.attestation_service import AttestationService

        payload = AttestationService._photo_payload_from_medical_dict(q_med.reponses)
        url = _normalize_photo_payload_to_url(payload)
        if url:
            return url
    return None
...
def enrich_alerte_with_related_data(
    ...
    if hasattr(alerte, "user") and alerte.user:
        ...
        user_photo = getattr(alerte.user, "photo_url", None)
        if not user_photo and db is not None:
            user_photo = _subscription_identity_photo_url(db, getattr(alerte, "souscription_id", None))
        setattr(alerte, "user_photo_url", user_photo)
...
async def trigger_sos(
    ...
    if not definitive_att:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Une attestation définitive est requise pour déclencher une alerte SOS (validation production en attente).",
        )
    ...
    # Trouver un agent sinistre disponible (SOS_OPERATOR ou AGENT_SINISTRE_MH)
    agent_sinistre = db.query(User).filter(
        User.role.in_([Role.SOS_OPERATOR, Role.AGENT_SINISTRE_MH]),
        User.is_active == True
    ).first()
    
    # Trouver le médecin référent prioritaire lié à l'hôpital assigné.
    medecin_referent = None
    if hospital and getattr(hospital, "medecin_referent_id", None):
        medecin_referent = db.query(User).filter(
            User.id == hospital.medecin_referent_id,
            User.role == Role.MEDECIN_REFERENT_MH,
            User.is_active == True
        ).first()

    # Fallback de continuité : référent MH actif global.
    ...
    # Dernier recours historique : doctor.
    if not medecin_referent:
        medecin_referent = db.query(User).filter(
    ...
    # puis déclencher les push APRÈS commit DB (sinon le worker peut ne pas voir la notification).
    pending_pushes: List[tuple[int, int]] = []
    ...
    if agent_sinistre:
        ...
        pending_pushes.append((agent_sinistre.id, notification_agent.id))
    ...
    if medecin_referent:
        ...
        # WebSocket médecin : après commit (voir plus bas) pour que GET /sos/{id} voie les données
        
        pending_pushes.append((medecin_referent.id, notification_medecin.id))
    ...
    for push_user_id, push_notification_id in pending_pushes:
        _enqueue_or_send_push_now(
    ...
    # Après commit : les autres sessions (et le front qui fait GET /sos/{id}) voient l'alerte
    if medecin_referent:
    ...
    await manager.broadcast_to_role(alert_ws_payload, Role.SOS_OPERATOR)
    await manager.broadcast_to_role(alert_ws_payload, Role.AGENT_SINISTRE_MH)

    enrich_alerte_with_related_data(alerte, sinistre, hospital, db)
...
def _hydrate_alertes_for_response(db: Session, alertes: List[Alerte]) -> bool:
    ...
    for alerte in alertes:
        ...
        steps = []
        if sinistre:
            steps_orm, wf_modified = ensure_workflow_steps(db, sinistre, alerte)
            if wf_modified:
                workflow_modified_any = True
...
async def get_alerte(
    ...
    if sinistre:
        steps_orm, workflow_modified = ensure_workflow_steps(db, sinistre, alerte)
        for s in sorted(steps_orm, key=lambda x: x.ordre):
            steps.append({
                "step_key": s.step_key,
                "titre": s.titre,

...
def _get_assureur_scope_for_accountant(db: Session, current_user: User) -> List[int]:
    """Résout les assureurs d'un comptable via les deux modes d'affectation supportés."""
    assureur_ids = {
        row[0]
    ...
    return sorted(assureur_ids)


def _get_courtier_scope_for_accountant(db: Session, current_user: User) -> List[int]:
    """Liste des courtiers explicitement affectés à l'agent comptable courtier."""
    return [
        row[0]
        for row in db.query(Courtier.id)
        .filter(Courtier.agent_comptable_id == current_user.id)
        .all()
    ]
...
def _dispatch_invoice_push_queue(push_queue: List[Tuple[int, int]]) -> None:
    """Push après commit : la tâche Celery lit `notifications` déjà persistée."""
    if not push_queue:
        return
    try:
        ...
        return
    for user_id, notification_id in push_queue:
        try:
...
def _notify_invoice_medical_review(
    db: Session,
    hospital: Hospital,
    titre: str,
    message: str,
    invoice_id: int,
    push_queue: List[Tuple[int, int]],
):
    """
    Revue médicale centrale + médecin référent de l'hôpital facturé (pas tous les référents MH).
    Types FCM référent : invoice_medical_review (voir app.core.referent_notifications).
    """
    targets: List[User] = []
    seen: set[int] = set()
    for u in (
        db.query(User)
        .filter(
            User.role == Role.MEDICAL_REVIEWER,
    ...
    for user in targets:
        notification = Notification(
            ...
            titre=titre,
            message=message,
            lien_relation_type="invoice",
            lien_relation_id=invoice_id,
        )
        db.add(notification)
        db.flush()
        push_queue.append((user.id, notification.id))
...
def _notify_role_users(
    ...
    send_push_channel: bool = False,
    push_queue: Optional[List[Tuple[int, int]]] = None,
):
    if not roles:
        return
    users = (
...
def _invoice_query_with_access(db: Session, current_user: User):
    ...
    if current_user.role in central_roles:
        if current_user.role == Role.AGENT_COMPTABLE_ASSUREUR:
            assureur_ids = _get_assureur_scope_for_accountant(db, current_user)
            if not assureur_ids:
        ...
        if current_user.role == Role.AGENT_COMPTABLE_COURTIER:
            courtier_ids = _get_courtier_scope_for_accountant(db, current_user)
            if not courtier_ids:
                return base_query.filter(false())
            return (
                base_query.join(Invoice.hospital_stay)
                .join(HospitalStay.sinistre)
                .join(Sinistre.souscription)
                .filter(Souscription.courtier_id.in_(courtier_ids))
            )
        return base_query
...
async def create_invoice(
    ...
    invoice_push_queue: List[Tuple[int, int]] = []
    try:
        ...
        message = (
            ...
            f"• Montant TTC: {invoice.montant_ttc:.2f} FCFA\n"
            f"• Cette facture nécessite votre validation médicale."
        )
        _notify_invoice_medical_review(
            db,
            hospital,
...
async def validate_medical(
    ...
    mark_notifications_read_for_relation(
        db,
        types=("invoice_medical_review",),
        lien_relation_type="invoice",
        lien_relation_id=invoice.id,
    )

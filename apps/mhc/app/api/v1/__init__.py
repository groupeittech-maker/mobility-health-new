...
async def health_check():
    """Health check pour le frontend (proxy Nginx). Réponse identique à /health."""
...
@api_router.get("/debug/ecard-config")
async def debug_ecard_config():
    """Diagnostic e-carte : indique si l’API utilise l’URL proxy (pour affichage dans le navigateur)."""
    from app.core.config import settings
    endpoint = (getattr(settings, "MINIO_ENDPOINT", None) or "").lower()
    base = (getattr(settings, "API_PUBLIC_BASE_URL", None) or "").strip()
    use_proxy = ("minio" in endpoint or "localhost" in endpoint or "127.0.0.1" in endpoint) and bool(base)
    return {
        "use_ecard_proxy": use_proxy,
        "API_PUBLIC_BASE_URL": base or "(vide)",
        "MINIO_ENDPOINT": endpoint or "(vide)",
        "message": "URL proxy utilisée pour l'e-carte" if use_proxy else "URL proxy NON utilisée (vérifier API_PUBLIC_BASE_URL et redémarrer le conteneur)",
    }
...
@api_router.get("/")
async def api_root():
    """Root endpoint for API v1 (avec ou sans slash final)."""
    from datetime import datetime
    import time
...
api_router.include_router(
    admin_tarification.router,
    prefix="/admin/tarification",

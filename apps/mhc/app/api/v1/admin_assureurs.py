...
@router.delete("/{assureur_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_assureur(
    ...
    linked_courtiers_rows = (
        db.query(Courtier.nom)
        .filter(Courtier.assureur_id == assureur_id)
        .order_by(Courtier.nom.asc())
        .all()
    )
    if linked_courtiers_rows:
        linked_courtiers = [row[0] for row in linked_courtiers_rows if row and row[0]]
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=(
                f"Suppression impossible pour l'assureur {assureur.nom} (ID {assureur.id}) : "
                f"courtiers liés = {', '.join(linked_courtiers)}."
            ),
        )

    linked_products_rows = (
        db.query(ProduitAssurance.nom)
        .filter(ProduitAssurance.assureur_id == assureur_id)
        .order_by(ProduitAssurance.nom.asc())
        .all()
    )
    if linked_products_rows:
        linked_products = [row[0] for row in linked_products_rows if row and row[0]]
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=(
                f"Suppression impossible pour l'assureur {assureur.nom} (ID {assureur.id}) : "
                f"produits liés = {', '.join(linked_products)}."
            ),
        )
...
async def get_available_agents(
    ...
    try:
        ...
        role_value = role_mapping.get(role) if isinstance(role, str) else role
        ...
        available_agents = [
            {
                ...
                "is_assigned": row.id in assigned_agent_ids,
            }
            for row in rows
            if row.id not in assigned_agent_ids
        ]

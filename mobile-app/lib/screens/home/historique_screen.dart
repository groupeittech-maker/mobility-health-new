...
/// Historique : souscriptions, alertes, hospitalisations – connecté au backend.
...
class _HistoriqueScreenState extends State<HistoriqueScreen> {
  int _selectedTab = 0; // 0 Souscriptions, 1 Alertes, 2 Hospitalisations
  static const _tabs = ['Souscriptions', 'Alertes', 'Hospitalisation'];
...
class _SouscriptionsListState extends State<_SouscriptionsList> {
  ...
  Future<void> _load() async {
    ...
    try {
      final list = await _api.getSubscriptions();
      if (mounted) setState(() {
        _list = list;
        _loading = false;
      });
...
class _AlertesListState extends State<_AlertesList> {
  ...
  Future<void> _load() async {
    ...
    try {
      final list = await _api.getAlertes();
      if (mounted) setState(() {
        _list = list;
        _loading = false;
      });
...
class _HospitalisationsListState extends State<_HospitalisationsList> {
  ...
  Future<void> _load() async {
    ...
    try {
      final list = await _api.getHospitalStays();
      if (mounted) setState(() {
        _list = list;
        _loading = false;
      });
  ...
  Widget build(BuildContext context) {
    ...
    if (_list.isEmpty) return const Center(child: Text('Aucune hospitalisation'));
    final bottomPadding = MediaQuery.of(context).padding.bottom + 24;
    return RefreshIndicator(
      onRefresh: _load,
      child: ListView.builder(

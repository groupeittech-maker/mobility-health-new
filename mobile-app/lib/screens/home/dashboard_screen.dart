...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final subs = await SubscriptionsService().getSubscriptions(limit: 1000);
      final attCount = await _fetchAttestationsCount();
      List<Map<String, dynamic>> assureurs = [];
      try {
        assureurs = await AssureursService().getAssureurs();
      } catch (_) {}
      if (!mounted) return;
  ...
  Future<int> _fetchAttestationsCount() async {
    try {
      final list = await ApiClient().get<List<dynamic>>(
